      ...
      const response = await fetch('/api/campaigns', {
        ...
        body: JSON.stringify({
          ...
          job_payload: {
            ...
            formats: currentBrief.parsed.format,
            skip_research: false,
            skip_image: !(currentBrief.parsed.format || []).some((f: string) => ['feed_static', 'multi_format'].includes(f)),
            skip_video: !(currentBrief.parsed.format || []).some((f: string) => ['reels', 'video', 'multi_format'].includes(f)),
            skip_carousel: !(currentBrief.parsed.format || []).some((f: string) => ['carousel', 'multi_format'].includes(f)),
          },
        }),
      });
